...
//contante dictionnaire de type MIME pour résoudre les extensions de fichiers images
const MIME_TYPES = { //utiliser Mime type pour généer extention du fichier
...
const storage = multer.diskStorage({//créer objet de configuration de multer avec fonction de multer diskstorage pour enregiter sur le disque avec 2 param
    ...
    filename: (req, file, callback) => { //fonction filename indique à multer d'utiliser le nom d'origine, de remplacer les espace par les _ 
        ...
        callback(null, name + Date.now() + '.' + extension); //appel call bac avec null car pas d'erreur et créer le file name en entier
    }
    });
      
module.exports = multer({storage: storage}).single('image'); 
//appel mutler methode auquel passe objet storage et appeler methode single pour indiquer qu'il s'agit d'un fichier unique de type image